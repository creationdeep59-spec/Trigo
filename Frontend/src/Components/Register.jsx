...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "phone" && !/^[0-9]{10}$/.test(identifier)) {
  ...
  return (
    <div className="flex min-h-[85vh] items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-xl2 border border-ink/5 bg-white p-8 shadow-xl">
        ...
        <p className="mt-1 text-sm text-muted">Order from your favourite local spots in minutes.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-ink/10 px-3 py-2.5 focus-within:border-chili">
              <UserIcon size={17} className="text-muted" />
              <input
                required
                value={name}
        ...
        <p className="mt-6 text-center text-sm text-muted">
          ...
          <Link to="/login" className="font-semibold text-chili hover:underline">
            Log in
          </Link>
        </p>
      </div>
